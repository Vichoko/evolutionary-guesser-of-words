...
public class NaturalSelection {
	...
	public boolean perfectIndividualExistence;
	...
	public NaturalSelection(List<Individual> population, String perfectGenes) {
		...
		this.perfectIndividualExistence = false;
		this.perfectGenes = perfectGenes;
	}

/**
 * Selects the 'selectRate' porcent of the top fitness individuals.
 * @param selectRate Number between 0 and 1, rate of survivors after genocide.
 */
 ...
 */
	public void sortByFitness() {
		...
		population.sort(new Comparator<Individual>(){ // sort by fitness descending
		...
		if (population.get(0).getGenesStr().equals(perfectGenes)){
			perfectIndividualExistence = true;
 ...
 * Grabs pairs of survivors randomly and mate them to create a new child.
 Until the population is recovered; i.e. populationSize childs are created.
 * @param mutationRate  Number between 0 and 1, probability of have a random mutation in a gene.
 */
	public void matingPhase(double mutationRate) {
		...
		for (int childIndex = 0; childIndex < populationSize;){
			...
			children.add(
					population.get(firstParent).sex(population.get(secondParent), mutationRate));
			childIndex++;
		}
		this.updateGeneration(children);
	}
	/**
	 * calculates the fitness of the individual, saving the result inside the individual fitness field.
	 * 
	 * @param i Individual which fitness will be calculated
	 ...
	 * @param newGeneration List of child individuals.
	 */
