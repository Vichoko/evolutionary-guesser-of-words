...
public class Individual {
	protected char[] genes; // in this case a word of 5 characters
	...
	public void setSon(Individual son) {
		this.son = son;
	}
	 ...
	 * @param genesNumber Number of genes to be generated
	 */
	public Individual(int genesNumber){
		this.genes = new char[genesNumber];
		for (int i = 0; i < genesNumber; i++){
			genes[i] = misc.getRandomChar();
		}
		this.son = null;
	}

	/**
	 * Pairs two individuals creating a Child.
	 * First it does a cross-over of the genes, then generate random mutations and finally returning the son.
	 * @param peer individual which will be peared with the actual one.
	 * @param mutationRate probability of generate mutation in one gene.
	 * @return son.
	 */
	public Individual sex(Individual peer, double mutationRate){
		...
		this.setSon(son); peer.setSon(son);
		return son;
	}
	
	
	/**
	 * Generate mutation in the genes of 'mutatingGenes' according to 'mutationRate'.
	 * @param mutatingGenes Genes that will be mutated.
	 * @param mutationRate probability of generate mutation in one gene.
	 * @return Genes after the mutation procedure.
	 */
	protected char[] mutate(char[] mutatingGenes, double mutationRate) {
		for (int genIndex = 0; genIndex < mutatingGenes.length; genIndex++){
			if (Math.random() < mutationRate){
				char mutation = misc.getRandomChar(); // get random char between 'a' and 'z' in ascii.
	 ...
	 * @param mommy peer gene container.
	 * @return son genes after cross over between father (this) and mother (param).
	 */
	char[] crossover(Individual mommy) {
